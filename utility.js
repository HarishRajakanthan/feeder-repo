...
async function fetchData() {
  ...
  try {
    // 1. Hit the Endpoint
    const response = await fetch('https://in.adda.io/actions/ajax_maintenance_trackfees_table.php?income_filter=4&choosen_block=All%20Blocks&shouldSearch=0&utility=1now=%22{dt}%20{tm}%22');
    ...
    rows.forEach((row) => {
      ...
      if (cells.length > 6) {
        ...
        if (!isNaN(dueAmount) && dueAmount >= 50) {
...
results.forEach(code => {
  ...
  result.push(value);
    ...
    // 5. Display Output
    if (results.length > 0) {
      outputDiv.textContent = "Dear residents, \n\nUtility payments for this month is still pending for the following units.Kindly clear the dues at the earliest\n"  + result.join('\n');
    } else {
